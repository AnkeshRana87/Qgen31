...
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error In Process Data: ' || SQLERRM);
        ROLLBACK;
         ...
         FOR org_rec IN (
                ...
                BEGIN
                    EGO_ITEM_PUB.assign_item_to_org(
                        ...
                        p_commit                 => FND_API.G_FALSE,
                        p_inventory_item_id      => x_item_table(i).inventory_item_id,
                        p_item_number            => x_item_table(i).segment1,
                        p_organization_id        => org_rec.target_org_id,
                        --p_organization_code      => org_rec.target_org_code,
                        --p_primary_uom_code       => org_rec.primary_uom_code,
                        --p_wip_supply_subinventory => org_rec.wip_supply_subinventory,
                        x_return_status          => x_return_status,
                        x_msg_count              => x_msg_count,
                        x_msg_data               => x_msg_data
                    );
                    ...
                    IF x_return_status <> FND_API.G_RET_STS_SUCCESS THEN
                        --DBMS_OUTPUT.PUT_LINE('Assignment failed: ' || x_msg_data);
						fnd_file.put_line (fnd_file.LOG,'Assignment failed: ' || x_msg_data ) ;

                    ELSE
                        --DBMS_OUTPUT.PUT_LINE('Item ' || x_item_table(i).segment1 || ' assigned to org ' || org_rec.target_org_id);
                ...
                EXCEPTION
                    WHEN OTHERS THEN
                        --DBMS_OUTPUT.PUT_LINE('Error in assignment: ' || SQLERRM);
 ...
 BEGIN
      ...
      LOOP
        ...
        -- TEMPLATE NAME --       
         ...
         --- DUPLICATE ITEM NUMBER IN STAGING ---
            ...
            BEGIN
                ...
                THEN 
                ITEM_TBL (i).PROCESS_FLAG := 'E';
                ITEM_TBL (i).ERROR_MESSAGE := 'Item Already Exists';
                END IF;
            END ;
		...
		BEGIN
			...
			AND RECORD_ID  = REQ_LINE_tbl (i).RECORD_ID;
			EXCEPTION
                  WHEN OTHERS
                  THEN
                     ITEM_TBL (i).PROCESS_FLAG := 'E';
