...
END IF;
   ...
   --EXECUTE_QUERY;
      ...
      BEGIN
				SELECT 
				NVL(PAAF.SUPERVISOR_ID,0) INTO LC_SUPERVISOR
				FROM FND_USER FU 
      	...
      	THEN
      	fnd_message.set_string (   'ERROR IN FETCHING SUPERVISOR '
                              || SQLCODE
                              || '-'
                              || SQLERRM
                             );
      ...
      UPDATE xxfil_bom_stg
         SET batch_id = ln_request_load,
             ...
             initiater = gn_user_id,
             ORG_ID = gn_org_id,
             status = 'NEW',
             APPROVER = LC_SUPERVISOR
       WHERE status IS NULL AND batch_id IS NULL;

			-- STORE THIS REQUEST ID IN GLOBAL VARIABLE
		  --	:GLOBAL.LOAD_BATCH_ID := ln_request_load ;
			
			--fnd_message.set_string ('LOAD_BATCH_ID : '||:GLOBAL.LOAD_BATCH_ID);
      --fnd_message.show;
      COMMIT;
...
PROCEDURE MAIL_TO_APPROVER IS
   ...
   commit;
      ...
      ELSIF ln_request > 0
      THEN
       fnd_message.set_string (ln_request);
      fnd_message.show;
      ...
      fnd_message.set_string (lc_phase||' , '||lc_status);
      fnd_message.show;
...
BEGIN
    ...
    fnd_message.set_name('FND', '	Are you sure You want to Approve');
...
BEGIN
    ...
    fnd_message.set_name('FND', '	Are you sure You want to Reject');
...
PROCEDURE DYNAMIC_APPROVAL IS
	ln_user number := fnd_profile.value('USER_ID');
begin
	if :XXFIL_BOM_UPLOAD_STG.BATCH_ID is not null 
		then 
		if :XXFIL_BOM_UPLOAD_STG.approver = ln_user
					then 
							SET_ITEM_PROPERTY('XXFIL_BOM_UPLOAD_STG.APPROVE',enabled,property_true);
							SET_ITEM_PROPERTY('XXFIL_BOM_UPLOAD_STG.REJECT',enabled,property_true);
							else
							SET_ITEM_PROPERTY('XXFIL_BOM_UPLOAD_STG.APPROVE',enabled,property_false);
							SET_ITEM_PROPERTY('XXFIL_BOM_UPLOAD_STG.REJECT',enabled,property_false);
					end if;
		else
		SET_ITEM_PROPERTY('XXFIL_BOM_UPLOAD_STG.REJECT',enabled,property_false);
		SET_ITEM_PROPERTY('XXFIL_BOM_UPLOAD_STG.APPROVE',enabled,property_false);
	end if;
end DYNAMIC_APPROVAL;
